#!/usr/bin/env python3
"""Create .spwiki content skeletons from a scan JSON file."""
...
def write_sidecar_configs(project_root: Path, force: bool, created: List[str], skipped: List[str]) -> None:
    for name, data in default_project_sidecar_files().items():
        write_json_file(project_root / ".spwiki" / name, data, force, created, skipped)
...
def create_for_project(
    ...
    if write_config:
        ...
        write_json_file(output_project_root / ".spwiki" / "config.json", config, force, created, skipped)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create .spwiki content skeletons from scan_project.py JSON.")
    ...
    parser.add_argument(
        "--language",
        default=None,
        help="Wiki language key, such as zh, en, or kr. Default: the detected system language. "
        "Once written to .spwiki/config.json this language is authoritative for all later maintenance.",
    )
    parser.add_argument("--force", action="store_true", help="Overwrite existing files.")
    parser.add_argument("--write-config", action="store_true", help="Write .spwiki/config.json and project sidecar config files.")
    parser.add_argument("--build-index", action="store_true", help="Build .spwiki/wiki-index.json after creating skeletons.")
    args = parser.parse_args()

    language = args.language or detect_system_language()
